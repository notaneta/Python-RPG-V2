...
class Personagem:
    ...
    def subirnivel(self, listahabilidades, listamagias): 
        while self.xp >= self.xplevel:
            ...
            for magia in listamagias:
                if magia.requisito <= self.nivel and skill not in self.magiasaprendidas:  # Além disso verifica se a magia ou skill já não foi aprendida, para evitar duplicidade
    ...
    def adicionaritem(self, novo_item):
...
class Heroi(Personagem):
    def __init__(self, nome):
        super().__init__(nome, 100, 10, 0, 40, 20, 1, 0, 40, 0) # Herdou os parametros de personagem / Aqui já configuro os status inicias também
