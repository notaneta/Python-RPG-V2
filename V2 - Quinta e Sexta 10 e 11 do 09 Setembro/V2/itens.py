...
poçoes = [
    ...
    Pocao("2","Poção Média", 75, 35, 1, "Cura media da sua vida"),
...
def usaritem(heroi):
    while True:
        ...
        if 0 <= indice < len(heroi.inventario):
            item_escolhido = heroi.iventario[indice]
            heroi.removeritem(item_escolhido)
            heroi.vida = min(heroi.vidamax, heroi.vida + item_escolhido.cura)
            print(f"O item {item_escolhido.nome} foi usado e curou {item_escolhido.cura} da sua vida")
